...
import LocationChanges, {
  DeliveryInformation,
} from "../ReduxComponent/PriceCalculations"
...
const Paystack = ({ handleBack, cart }) => {
  ...
  const handleLocationChange = (e) => {
    const selectedLocation = locationData.find(
      (loc) => loc.exactLocation === e.target.value
    )
    if (selectedLocation) {
  ...
  return (
    <section className="text-sm capitalize ">
      <div className="flex justify-between mb-7 lg:mb-3 items-center">
        <BiLeftArrowCircle fontSize={25} onClick={handleBack} />
        <h1 className="text-xl font-600">Checkout</h1>
      </div>
      <div className="orders lg:h-[190px]  overflow-y-auto">
        {cart
          .slice()
          .reverse()
          .map((item, idx) => (
      ...
      <h3 className="flex text-lg font-700  my-2 justify-between">
        <p className="">Total:</p> <p>₦{totalPrice.toFixed(2)}</p>
      </h3>
      <form>
